                ...
                <tr v-for="(item, i) in desserts" :key="item.id">
                  ...
                  <td v-for="(detail,ki) in item.trackingDetail" :key="ki">
                    <v-tooltip bottom dark>
                      <template v-slot:activator="{ on }">
                        <v-icon
                          :color="getColor(detail.isWarning)"
                          large
                          v-on="on"
                      ...
                      <span :color="getColor1(detail.changeValue)">
                        <v-icon
                          :color="getColor1(detail.changeValue)"
                        >{{getIcon(detail.changeValue)}}</v-icon>
                    ...
                    <v-dialog v-model="dialog1" width="1000">
                      <template v-slot:activator="{ on }">
                        <v-btn text v-on="on" @click="getApi1(item.customerId)">
                          <v-icon
                            large
                            :color="getColor2(item.numOfDisapprovedAds)"
                          >fiber_manual_record</v-icon>
                    ...
                    <v-dialog v-model="dialog2" width="1000">
                      <template v-slot:activator="{ on }">
                        <v-btn text v-on="on" @click="getApi2(item.customerId)">
                          <v-icon
                            large
                            :color="getColor2(item.numOfAdGroupMissingAds)"
                          >fiber_manual_record</v-icon>
                      ...
                      <v-card>
                        <v-card-title
                          ...
                          primary-title
                        >AdGroup bị thiếu mẫu quảng cáo</v-card-title>
                        <v-card-text>
                          <v-simple-table>
                            <thead>
                              <tr style="color:blue; font-size: 20px;">
                    ...
                    <v-dialog v-model="dialog3" width="1000">
                      <template v-slot:activator="{ on }">
                        <v-btn text v-on="on" @click="getApi3(item.customerId)">
                          <v-icon
                            large
                            :color="getColor2(item.numOfAdGroupMissingExtensions)"
                          >fiber_manual_record</v-icon>
                          ...
                          <v-simple-table>
                            ...
                            <tbody>
                              <tr v-for="(item, c) in desserts3" :key="item.id">
                                ...
                                <td>
                                  <v-icon
                                    :color="getcolorIcon(item.missingCallExtension)"
                                  >{{getIcon1(item.missingCallExtension)}}</v-icon>
                                </td>
                                <td>
                                  <v-icon
                                    :color="getcolorIcon(item.missingCalloutExtension)"
                                  >{{getIcon1(item.missingCalloutExtension)}}</v-icon>
                                </td>
                                <td>
                                  <v-icon
                                    :color="getcolorIcon(item.missingSiteLinkExtension)"
